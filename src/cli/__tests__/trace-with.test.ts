...
describe('lux trace --with', () => {
  ...
  it('emits a federated trace usage event with attributes.federated + sibling names (SC-10)', () => {
    const res = runCli(corpus, ['--corpus', corpus, 'trace', SHOW, '--with', 'core', '--json']);
    expect(res.status).toBe(0);
    const db = new LuxDatabase(join(corpus, '.lux', 'lux.db'));
    try {
      const usage = db.getRecentEvents(10).find((e) => e.event_type === 'lux_usage_event');
      expect(usage).toBeDefined();
      const payload = JSON.parse(usage!.payload!) as {
        surface: string;
        attributes?: { federated?: boolean; with?: string[] };
      };
      expect(payload.surface).toBe('trace');
